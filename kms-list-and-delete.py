...
def is_aws_managed_key(key, aliases):
    ...
    # Check KeyManager field - most reliable indicator
    key_manager = key.get('KeyManager', '').upper()
    if key_manager == 'AWS':
        return True
    ...
    try:
        ...
        for i, key in enumerate(all_keys, 1):
            try:
                ...
                # Get full key metadata from key_details
                key_metadata = key_details.get('key_metadata', key)
                
                # Skip AWS-managed keys (check after getting aliases and metadata)
                if is_aws_managed_key(key_metadata, aliases):
                    continue
                ...
                should_expire = should_expire_key(tags, exclude_vector, key_metadata, aliases)
                
                # Get deletion reason
                deletion_reason = get_key_deletion_reason(tags, exclude_vector, key_metadata, aliases)
                
                # Format key info with deletion info and reason
                key_info = format_key_info(key_metadata, aliases, tags, should_expire, deletion_info, deletion_reason)
